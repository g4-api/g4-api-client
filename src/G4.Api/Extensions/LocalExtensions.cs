...
namespace G4.Extensions
{
    internal static class LocalExtensions
    {
        ...
        extension(AutomationQueueModel queueModel)
        {
            /// <summary>
            ...
            public JobEventArgs Complete(JobEventArgs args)
            {
            ...
            public StageEventArgs Complete(StageEventArgs args)
            {
            ...
            public AutomationEventArgs Complete(AutomationEventArgs args)
            {
            ...
            public JobEventArgs Complete(RuleEventArgs args)
            {
            ...
            /// </returns>
            public (string Key, G4AutomationResponseModel Response) NewResponse(G4AutomationResponseModel response)
            {
            ...
            public RuleEventArgs UpdateStatus(RuleEventArgs args)
            {
                // Extract the identifiers for the stage, job, and rule from the event arguments
                var stageId = args.Rule.Reference.JobReference.StageReference.Id;
                var jobId = args.Rule.Reference.JobReference.Id;
            ...
            public JobEventArgs UpdateStatus(JobEventArgs args)
            {
                // Extract the identifiers for the stage and job from the event arguments
                var stageId = args.Stage.Reference.Id;
                var jobId = args.Job.Reference.Id;
            ...
            public StageEventArgs UpdateStatus(StageEventArgs args)
            {
            ...
            public AutomationEventArgs UpdateStatus(AutomationEventArgs args)
            {
        ...
        extension(ConcurrentDictionary<string, ConcurrentDictionary<string, AutomationQueueModel>> queue)
        {
            /// <summary>
            ...
            public void Add(AutomationQueueModel queueModel)
            {
            ...
            public bool TryDequeue(out AutomationQueueModel queueModel)
            {
            ...
            /// <param name="queueModel">The dequeued <see cref="AutomationQueueModel"/> if successful; otherwise, null.</param>
            /// <returns><c>true</c> if a <see cref="AutomationQueueModel"/> was successfully dequeued; otherwise, <c>false</c>.</returns>
            public bool TryDequeue(string group, out AutomationQueueModel queueModel)
            {
            ...
            /// <param name="queueModel">The dequeued <see cref="AutomationQueueModel"/> if successful; otherwise, null.</param>
            /// <returns><c>true</c> if a <see cref="AutomationQueueModel"/> was successfully dequeued; otherwise, <c>false</c>.</returns>
            public bool TryDequeue(string group, string id, out AutomationQueueModel queueModel)
            {
                // Initialize the output parameter.
                queueModel = null;
            ...
            public bool Update(AutomationQueueModel newModel)
            {
            ...
            public bool Update(string group, string id, Func<AutomationQueueModel, AutomationQueueModel> updateFactory)
            {
                // Check if the specified group exists.
                if (!queue.TryGetValue(group, out var innerQueue))
                {
            ...
            /// <returns><c>true</c> if the item was successfully updated; otherwise, <c>false</c>.</returns>
            public bool Update(string group, string id, AutomationQueueModel newModel)
            {
                // Check if the specified group exists.
                if (!queue.TryGetValue(group, out var innerQueue))
                {
        ...
        extension(ConcurrentDictionary<string, ConcurrentDictionary<string, G4QueueModel>> queue)
        {
            /// <summary>
            /// Adds or updates a <see cref="G4QueueModel"/> in the concurrent queue based on its group and identifier.
            /// </summary>
            /// <param name="queueModel">The queue model to add or update.</param>
            public void Add(G4QueueModel queueModel)
            {
                // Extract the progress status identifier from the queue model.
            ...
            public bool TryDequeue(out G4QueueModel queueModel)
            {
                // Initialize the output parameter to null.
                queueModel = null;
            ...
            /// <param name="queueModel">The dequeued <see cref="G4QueueModel"/> if successful; otherwise, null.</param>
            /// <returns><c>true</c> if a <see cref="G4QueueModel"/> was successfully dequeued; otherwise, <c>false</c>.</returns>
            public bool TryDequeue(string group, out G4QueueModel queueModel)
            {
                // Initialize the output parameter.
                queueModel = null;
            ...
            /// <param name="queueModel">The dequeued <see cref="G4QueueModel"/> if successful; otherwise, null.</param>
            /// <returns><c>true</c> if a <see cref="G4QueueModel"/> was successfully dequeued; otherwise, <c>false</c>.</returns>
            public bool TryDequeue(string group, string id, out G4QueueModel queueModel)
            {
                // Initialize the output parameter.
                queueModel = null;
            ...
            /// <returns><c>true</c> if the update was successful; otherwise, <c>false</c>.</returns>
            public bool Update(G4QueueModel newModel)
            {
                // Check if the new model contains a valid group identifier.
            ...
            /// <returns><c>true</c> if the item was successfully updated; otherwise, <c>false</c>.</returns>
            public bool Update(string group, string id, Func<G4QueueModel, G4QueueModel> updateFactory)
            {
                // Check if the specified group exists.
                if (!queue.TryGetValue(group, out var innerQueue))
                {
            ...
            /// <returns><c>true</c> if the item was successfully updated; otherwise, <c>false</c>.</returns>
            public bool Update(string group, string id, G4QueueModel newModel)
            {
                // Check if the specified group exists.
                if (!queue.TryGetValue(group, out var innerQueue))
                {
        ...
        extension(G4AutomationModel automation)
        {
            ...
            public IEnumerable<(G4AutomationModel Automation, IDictionary<string, object> DataProvider)> NewAutomations()
            {
            ...
            public ParallelOptions NewParallelOptions(CancellationToken cancellationToken)
            {
            ...
            public G4QueueModel NewQueueModel()
            {
            ...
            public G4QueueModel NewQueueModel(int status)
            {
            ...
            public G4QueueModel NewQueueModel(IDictionary<string, object> properties)
            {
            ...
            public G4QueueModel NewQueueModel(int status, IDictionary<string, object> properties)
            {
        ...
        extension(G4AutomationStatusModel queueStatus)
        {
            /// <summary>
            ...
            /// <returns>A <see cref="double"/> representing the progress percentage. Returns <c>0.0</c> if no stages are completed to avoid division by zero.</returns>
            public double MeasureProgress(G4AutomationModel automation)
            {
        ...
        extension(G4AutomationStatusModel.StageStatusModel stageStatus)
        {
            /// <summary>
            ...
            public double MeasureProgress(G4StageModel stage)
            {
        ...
        extension(G4AutomationStatusModel.JobStatusModel jobStatus)
        {
            /// <summary>
            /// Calculates the progress of a job based on the total number of rules and the number of completed rules.
            /// </summary>
            /// <returns>A <see cref="double"/> representing the progress percentage. Returns <c>0.0</c> if no rules are completed to avoid division by zero.</returns>
        ...
        extension(G4AutomationStatusModel.PluginStatusModel ruleStatus)
        {
            /// <summary>
        ...
        extension(G4DataProviderModel dataProvider)
        {
            /// <summary>
        ...
        extension(IEnumerable<(string GroupId, G4AutomationResponseModel Response)> responseCollection)
        {
            /// <summary>
            /// Creates a new automation response based on a collection of automation responses.
            /// </summary>
            /// <returns>A dictionary where each key is a group ID and the value is the merged automation response model for that group.</returns>
            public IDictionary<string, G4AutomationResponseModel> NewAutomationResponse()
            {
        ...
        extension(IGrouping<string, (string Key, G4AutomationResponseModel Response)> group)
        {
            /// <summary>
            ...
            /// </returns>
            public (string Key, G4AutomationResponseModel Response) NewAutomationResponse()
            {
